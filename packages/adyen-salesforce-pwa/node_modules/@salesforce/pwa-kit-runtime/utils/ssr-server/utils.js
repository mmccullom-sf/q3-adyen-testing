...
const catchAndLog = (err, context) => {
  ...
  console.error(message, /* istanbul ignore next */
...
const getFullRequestURL = url => {
  ...
  const proxy = _ssrShared.proxyConfigs.find(({
    path
  }) => url.startsWith(`${_constants.PROXY_PATH_PREFIX}/${path}/`));
  if (proxy) {
    return url.replace(`${_constants.PROXY_PATH_PREFIX}/${proxy.path}`, `${proxy.protocol}://${proxy.host}`);
  }
  throw new Error(`Unable to fetch ${url}, relative paths must begin with ${_constants.PROXY_PATH_PREFIX} followed by a configured proxy path.`);
};
